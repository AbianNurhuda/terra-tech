...
export function PortfolioPage() {
  ...
  const loadPortfolios = async () => {
    setIsLoading(true)
    const response = await portfolioService.getPublicPortfolios()
    if (response.success) {
  ...
  const activePortfolios = portfolioList
    ...
    .map((p) => {
      ...
      return {
        ...
        client: p.client_name || originalProject.client || "Klien Terra Tech",
        year: p.created_at ? new Date(p.created_at).getFullYear().toString() : originalProject.year || new Date().getFullYear().toString(),
        fullDescription: p.description || originalProject.fullDescription || "",
        gallery: p.gallery_image_urls?.length ? p.gallery_image_urls : [p.cover_image_url]
      }
    })
  ...
  return (
    <main className="relative pt-28">
      ...
      <section ref={ref} className="container pb-14">
        {isLoading && <div className="py-16 text-center text-text-secondary">Memuat portfolio...</div>}
        {!isLoading && error && (
          <div className="py-16 text-center">
            <p className="text-rose-400 mb-4">{error}</p>
            <button onClick={loadPortfolios} className="px-4 py-2 rounded-xl bg-accent-cyan text-dark-base font-semibold">Coba lagi</button>
          </div>
        )}
        {!isLoading && !error && (
        <>
